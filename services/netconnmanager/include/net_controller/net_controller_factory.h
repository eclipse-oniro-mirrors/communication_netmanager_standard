...
public:
    sptr<INetController> MakeNetController(uint32_t netType);

private:
    sptr<INetController> GetNetControllerFromMap(uint32_t netType);
    std::map<uint32_t, sptr<INetController>> netControllers;
};
} // namespace NetManagerStandard
} // namespace OHOS
